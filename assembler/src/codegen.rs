...
pub fn codegen<'s>(statements: Vec<WithSpan<Stmt<'s>>>) -> Result<Vec<u8>, CodegenError<'s>> {
    ...
    for WithSpan { span, value: stmt } in statements.iter() {
        match stmt {
            Stmt::AliasDefinition(name, thing) => {
                alias_map.insert(name, *thing);
            }
            Stmt::RawDataDefinition(data) => offset += data.len() as u16,
            Stmt::Label(name) => {
                alias_map.insert(name, AliasableThing::RawData(offset));
            }
    ...
    macro_rules! resolve_addr {
        ($arg:ident) => {
            match $arg {
                OrAlias::Alias(alias) => match *alias_map
                    .get(alias)
                    .ok_or(CodegenError::AliasNotDefined(alias))?
                {
                    AliasableThing::RawData(data) => data,
                    AliasableThing::Register(_) => {
                        return Err(CodegenError::AliasShouldBeNumber(alias));
                    }
                },
            ...
            match $arg {
                    ...
                    .ok_or(CodegenError::AliasNotDefined(alias))?
                {
                    AliasableThing::RawData(_) => {
                        return Err(CodegenError::AliasShouldBeRegister(alias));
                    }
            ...
            match $arg {
                    ...
                    .ok_or(CodegenError::AliasNotDefined(alias))?
                {
                    AliasableThing::RawData(data) => {
                        if data > 0xFF {
                            return Err(CodegenError::AliasedLiteralTooBig(alias, data, 0xFF));
                        }
    ...
    for WithSpan { span, value: stmt } in statements.into_iter() {
        match stmt {
            ...
            Stmt::PseudoInstruction(instr) => {
                    ...
                    PI::Rnd(reg, mask) => {
                        ...
                        let mask = match mask {
                                ...
                                .ok_or(CodegenError::AliasNotDefined(alias))?
                            {
                                AliasableThing::RawData(data) => {
                                    if data > 0xFF {
                                        return Err(CodegenError::AliasedLiteralTooBig(
                                            alias, data, 0xFF,
                                        ));
                                    }
                                    data as u8
                ...
                blob.extend(encode(instruction)?);
            }
